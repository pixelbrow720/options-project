...
# ── Rev 5: public-site auth (Discord OAuth) ─────────────────────────────────


class DiscordStartResponse(BaseModel):
    url: str
    state: str
...
class AccessPendingResponse(BaseModel):
    """Returned with HTTP 403 when a user is authenticated but not approved."""
...
class AccessApproveRequest(BaseModel):
    api_key_id: UUID | None = None
    """Existing API key to assign. When omitted, an API key is auto-created."""
    allowed_symbols: list[str] | None = None
    """ACL applied to the auto-created API key. Defaults to ``["SPXW","NDXP"]``."""
...
class AccessApproveResponse(BaseModel):
    user: PublicUserSummary
    api_key: ApiKeySummary
    plaintext_key: str | None = Field(
        default=None,
        description="Plaintext API key — only set when the key was just generated.",
    )
