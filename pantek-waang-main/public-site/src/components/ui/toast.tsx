  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border border-border bg-card p-4 pr-6 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "bg-card text-card-foreground border-l-4 border-l-emerald",
        destructive:
          "bg-card text-card-foreground border-l-4 border-l-destructive",
      },
    },
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-1.5 top-1.5 rounded-md p-1 text-muted-foreground opacity-70 transition-opacity hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-1",
      className,
    )}
    toast-close=""
    {...props}
...
export function toast(opts: Omit<ToastEntry, "id">): number {
  const id = nextId++;
  const entry: ToastEntry = { id, duration: 4000, ...opts };
  toasts = [...toasts, entry];
  emit();
  return id;
}
