...
### Migrations
Alembic auto-runs on backend container startup. Manually: `cd backend && alembic upgrade head`. Versions live in `app/db/migrations/versions/` (currently 0001 → 0011). Operationally-meaningful migrations:
...
- **0011** swaps `flow_events` ascending `(symbol, ts)` index for descending `(symbol, ts DESC)` to match every `ORDER BY ts DESC LIMIT N` read path. Adds nullable `databento_api_keys.last_test_at` for admin UI freshness display.
...
- **Weight-source provenance rule (Rev 7).** Any metric with a fallback chain MUST persist `weight_source` in `extra_json`. GEX, walls, and HIRO all comply. Adding a new fallback-chain metric without this is a measurement-correctness blind spot — the regime score consumes wall totals and would silently flip on uniform-fallback ranking otherwise.
- **`TAU_FLOOR_YEARS` is canonical in `app/processing/session.py`** — 15 minutes. Never redefine locally. Pin probability and vanna/charm both import from there.
- **`is_expiration_day(symbol, *, today=None, available_expirations=None)`** — chain-aware. Resolution order: explicit `available_expirations` arg → per-symbol module cache populated by `set_available_expirations(symbol, expirations)` (pipeline writes the chain's expiry set every tick) → static M/W/F fallback. Don't rely on the static set; SPXW lists Tue + Thu weeklies.
- **`bsm.charm` returns delta-change-per-year.** When mixing with count-based weights (OI, volume), scale by τ first to keep the ratio dimensionally coherent and τ-stable as expiry approaches. See `pin_probability.py` for the canonical pattern.
- Read `PROJECT_OVERVIEW.md` for the deeper structural map; `REVIEW.md` for the latest hardening pass and `FIXES_REV7.md` for the most recent closure log.
