...
async def build_snapshot_payload(session: AsyncSession, symbol: str) -> tuple[dict[str, Any], datetime | None]:
    ...
    # snapshot tail). Single round-trip via ``count(*) OVER ()`` window —
    # the count is identical on every returned row, so we read it from
    # the first row (or default to 0 when the window is empty).
    ...
    flow_result = (await session.execute(flow_stmt)).all()
