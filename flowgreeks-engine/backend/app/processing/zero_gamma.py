...
from app.processing.session import TAU_FLOOR_YEARS, calendar_tau_years
    ...
    *,
    ...
    Args:
        fallback_to_closest: When True, if the grid scan never crosses zero,
            ...
            ``None`` return.
        tau_years: Optional session-aware τ override. When provided, every
            row whose calendar τ collapses to the 1-day floor (i.e. an
            expiration equal to ``today``) has its τ replaced with this
            value, floored at :data:`TAU_FLOOR_YEARS`. Mirrors the pattern
            in :mod:`app.processing.vanna_charm`. Without this override a
            13:00 ET 0DTE-heavy SPX chain reports a flip level computed at
            τ ≈ 1/365 instead of ~3-4× smaller — the crossing shifts and
            the reported level is a calendar-floor artifact.
    """
    ...
    # 0DTE override: when the caller passes a session-aware τ (e.g. the
    # intraday τ from session.time_to_expiry_0dte_years), substitute it
    # for every row whose expiration is today. Without this the gamma
    # grid for 0DTE rows runs at the 1-day calendar floor (τ ≈ 2.74e-3)
    # which over-states their contribution by ~√7 in the afternoon.
    if tau_years is not None and np.isfinite(tau_years) and tau_years > 0:
        same_day_mask = (
            pd.to_datetime(work["expiration"], errors="coerce").dt.date == today_d
        )
        if same_day_mask.any():
            override = max(TAU_FLOOR_YEARS, float(tau_years))
            work.loc[same_day_mask, "tau"] = override
    work = work[
        (work["weight"].abs() > 0)
