...
def test_ws_stream_revocation_closes_with_4401(monkeypatch) -> None:
    """WS connection must close with code 4401 once the watcher detects revocation.

    The watcher polls api_keys every ``REVOCATION_CHECK_INTERVAL_SECONDS``;
    we shrink that knob and use a fake session whose ``get`` returns None
    (mirroring an admin-deleted row) so ``_is_revoked`` flips True on the
    first tick.
    """
    from starlette.websockets import WebSocketDisconnect
    ...
    class _FakeApiKey:
        id = "ws-revocation-key"
        is_active = True
        expires_at = None
        allowed_symbols = ["SPXW"]

    async def _fake_auth(api_key, symbol, session):  # noqa: ARG001
        return _FakeApiKey() if api_key == "ak_ws_revocation" else None
    ...
    monkeypatch.setattr(stream_mod, "_authenticate_streaming_key", _fake_auth)
    monkeypatch.setattr(stream_mod, "build_snapshot_payload", _empty_payload)
    monkeypatch.setattr(stream_mod, "REVOCATION_CHECK_INTERVAL_SECONDS", 0.05)

    client = _build_ws_test_client()
    plaintext = "ak_ws_revocation"
    try:
        with pytest.raises(WebSocketDisconnect) as excinfo:
            with client.websocket_connect(
                "/v1/SPXW/stream", headers={"X-API-Key": plaintext}
                ...
                while True:
                    ws.receive_text()
        assert excinfo.value.code == stream_mod.WS_REVOKED_CODE
